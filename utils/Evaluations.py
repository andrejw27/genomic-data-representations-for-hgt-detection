...
class Evaluations:
    ...
    def evaluations_test(self, total_orgs, models, gi_eval, organism_neg_dict, save_result=True):
        evaluation_result = {}
        for key in models.keys():
            ...
            print("recall = ", rec)
            evaluation_result.update({key:org_score})
        if save_result:
            print('download result')
            self.result_download(evaluation_result, 'test')

        return evaluation_result
    
    def evaluations_main_104(self, total_orgs, models, gi_eval, organism_neg_dict, result_type, save_result=True):
        evaluation_result = {}
        for key in models.keys():
            model = models[key]
            org_num = len(total_orgs)
    ...
    def result_download(self, org_score, result_type):
        # with open("evaluation_result_" + str(result_type),"wb") as fp:  # Pickling
        ...
        json_obj = json.dumps(org_score, indent=1, default=myconverter)
        json_file = "evaluation_result_{}.json".format(result_type)
